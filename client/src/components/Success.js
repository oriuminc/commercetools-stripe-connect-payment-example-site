...
const Success = () => {
  ...
  const [{ isCapturing, isCanceling, isRefunding }, setFetchingState] =
    useState({
      isCapturing: false,
      isCanceling: false,
      isRefunding: false,
  ...
  useEffect(() => {
    ...
    const asyncCall = async () => {
      let { latest_charge } = await fetch(
        `${BACKEND_URL}/payment-intent/${payment_intent}`
      ).then((res) => res.json());
  ...
  const requestRefund = async () => {
    ...
    await fetch(`${BACKEND_URL}/request-refund/`, {
      ...
      .then((res) => res.json())
      .catch((_) =>
        setFetchingState((prev) => ({ ...prev, isRefunding: false }))
      );
  ...
  const cancelPayment = async () => {
    ...
    await fetch(`${BACKEND_URL}/cancel-payment/`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...DEV_REQUEST_HEADERS,
  ...
  return (
    <>
      ...
      {isRefunded && !isCanceled && "The payment is Refunded"}
      ...
      <div className="flex gap-4 pt-4">
          ...
          <button
            ...
            className={`${
              !isCapturing
                ...
                : "border-[#9d9dad] text-[#9d9dad]"
            } px-3 py-2 rounded-md font-medium border-2 `}
          >
        ...
        {(isCaptured || capture_method.current !== "manual") && !isRefunded && (
          <button
            ...
            className={`${
              !isCanceling
                ? "border-[#635bff] text-[#635bff]"
                : "text-[#9d9dad] border-[#9d9dad]"
            } px-3 py-2 rounded-md font-medium border-2`}
          >
            {isRefunding ? <Spinner /> : "Request Refund"}
          </button>
        )}
        {!isCanceled && capture_method.current === "manual" && !isCaptured && (
          <button
            ...
            className={`${
              !isCanceling ? "bg-[#ec1313]" : "bg-[#9d9dad]"
            } px-3 py-2 rounded-md text-white font-medium`}
          >
            {isCanceling ? <Spinner /> : "Cancel Payment Authorization"}
          </button>
        )}
      </div>
    </>
